...
public class List3_2 {
    ...
    public static void main(String[] args) {
        ...
        for (Person p : personList) {
            Optional.ofNullable(p.getZipCode())
                    .map(t -> zipCodeMap.get(t))
                    .map(t -> {
                        int lastIndex = t.length() < 8 ? t.length() : 8;
                        return t.substring(3, lastIndex);
                    })
                    .ifPresent(t -> System.out.println("郵便番号" + p.getZipCode() + "の住所の先頭5文字は" + t + "です。"));
        }
    }
